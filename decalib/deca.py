...
class DECA(nn.Module):
    ...
    def encode(self, images, use_detail=True):
        ...
        if self.cfg.model.jaw_type == 'euler':
            ...
            print("Before modification - posecode:", posecode)
            ...
            print("After modification - posecode:", posecode)
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
                render_orig=False, original_image=None, tform=None, default_pose=None):
        images = codedict['images']
        batch_size = images.shape[0]
        print("+++++++++++++++++++++++codedict[pose]:", default_pose,"//", codedict['pose'])
        
        ## decode
        #verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])
        
        if default_pose is not None:
        # Use default pose instead of pose parameters extracted from the input image
            verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=default_pose)
        else:
            verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])
